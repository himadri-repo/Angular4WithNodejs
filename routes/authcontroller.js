...
router.use(function(req, res, next) {
    ...
    var permission = getPermission(config.authRules, req.originalUrl);

    // if(req.originalUrl.indexOf('register')>-1 || 
    //     req.originalUrl.indexOf('amivalid')>-1) {
    //     next();
    // }
    if(permission && !permission.isDenied) {
        next();
    }
    else
    {
        if(!token) {
            res.status(permission.statusCode).send({'status': false, 'message': 'Invalid or missing token'});
            return;
            //res.end();
            //next();
        }
...
var getPermission = function(rules, url) {
    var permission = 'deny';
    var statusCode = 401;
